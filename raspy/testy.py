...
def vector_to_latlon(forward, up):
    """
    Convert sensor vectors to latitude and longitude in a mini globe.
    Latitude comes from up vector (gravity),
    Longitude comes from rotation of forward vector around up vector.
    """
    # Normalize vectors
    ...
    # Longitude: project forward onto horizontal plane
    if abs(lat) > 89.999:  # near poles, longitude undefined
        lon = 0.0
    else:
        # Horizontal forward vector
        hx = fx - (fx*ux + fy*uy + fz*uz)*ux
        hy = fy - (fx*ux + fy*uy + fz*uz)*uy
        # atan2 gives full 360° rotation around up
